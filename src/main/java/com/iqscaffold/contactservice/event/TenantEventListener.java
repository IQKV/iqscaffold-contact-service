 ...
 * Handles tenant lifecycle events to provision contact database schemas.
 */
@Component
public class TenantEventListener {
   ...
   * Provisions contact database schema when tenant is created.
   */
  @RabbitListener(queues = "iqscaffold.contact.tenant.events")
  ...
  private void handleTenantCreated(TenantEvent event) {
    ...
    try {
      ...
      log.info("Successfully provisioned contact schema for tenant: {} (organization: {})",
          event.getTenantId(), event.getOrganizationName());

    } catch (final Exception e) {
      log.error("Failed to provision contact schema for tenant: {}", event.getTenantId(), e);
      throw new RuntimeException("Failed to provision contact schema for tenant: " + event.getTenantId(), e);
    }
  }
   ...
   * Archives contact data but does not drop schema (for compliance).
   */
  private void handleTenantDeleted(TenantEvent event) {
    ...
    // - Historical contact data retention
    // - Potential data recovery needs
    // - Legal/business record keeping

    log.warn("Tenant deleted event received for: {}. Contact data archived but schema retained for compliance.",
        event.getTenantId());
   ...
   * Suspends contact operations for the tenant (reversible).
   */
  private void handleTenantSuspended(TenantEvent event) {
    ...
    try {
      // Suspend contact operations for this tenant
      // - Stop accepting new contact creation
      // - Mark existing contacts as suspended
      // - Maintain schema and data for restoration

      log.info("Successfully suspended contact operations for tenant: {}", event.getTenantId());
    } catch (final Exception e) {
      log.error("Failed to suspend contact operations for tenant: {}", event.getTenantId(), e);
      throw new RuntimeException("Failed to suspend contact operations for tenant: " + event.getTenantId(), e);
    }
  }

  /**
   * Handle tenant archived event.
   * Archives contact data and prevents future operations (terminal state).
   */
  private void handleTenantArchived(TenantEvent event) {
    ...
    try {
      // Archive contact data for this tenant
      // - Disable all contact operations
      // - Archive contact records
      // - Retain schema for compliance and audit
      // - This is a terminal state - no restoration possible

      log.warn("Successfully archived contact data for tenant: {}", event.getTenantId());
    } catch (final Exception e) {
      log.error("Failed to archive contact data for tenant: {}", event.getTenantId(), e);
      throw new RuntimeException("Failed to archive contact data for tenant: " + event.getTenantId(), e);
    }
  }

  /**
   * Handle tenant restored event.
   * Restores contact operations for the tenant from suspended state.
   */
  private void handleTenantRestored(TenantEvent event) {
    ...
    try {
      // Restore contact operations for this tenant
      // - Re-enable contact creation and updates
      // - Restore contact processing
      // - Resume normal operations

      log.info("Successfully restored contact operations for tenant: {}", event.getTenantId());
    } catch (final Exception e) {
      log.error("Failed to restore contact operations for tenant: {}", event.getTenantId(), e);
      throw new RuntimeException("Failed to restore contact operations for tenant: " + event.getTenantId(), e);
    }
  }
