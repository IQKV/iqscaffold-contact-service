...
| `INFRA_POSTGRESQL_PASSWORD`       | PostgreSQL database password         | Contact data storage                       |
| `INFRA_REDIS_PASSWORD`            | Redis cache password                 | Contact caching, session management        |
| `INFRA_RABBITMQ_PASSWORD`         | RabbitMQ message broker password     | Contact lifecycle event messaging          |
...
drone secret add --repository IQKV/iqscaffold-contact-service --name INFRA_POSTGRESQL_PASSWORD --data "your-postgresql-password"
drone secret add --repository IQKV/iqscaffold-contact-service --name INFRA_REDIS_PASSWORD --data "your-redis-password"
drone secret add --repository IQKV/iqscaffold-contact-service --name INFRA_RABBITMQ_PASSWORD --data "your-rabbitmq-password"
drone secret add --repository IQKV/iqscaffold-contact-service --name JWT_SECRET_KEY --data "your-secure-symmetric-key"
```
...
| `JWT_SECRET_KEY`            | `config.crm.security.jwt.secretKey` | `JWT_SECRET_KEY`                 | JWT symmetric validation secret  |
...
cd charts/IQKV/iqscaffold-contact-service
...
| JWT Secret Key      | `JWT_SECRET_KEY`            | ✅       | JWT symmetric validation key (HS256) |

### External Services

The service connects to these external infrastructure components:

- **PostgreSQL**: Contact data storage (database: `iqscaffold_contact`, Redis DB: 2)
- **Redis**: Contact caching and session management
- **RabbitMQ**: Event messaging for contact lifecycle events
- **User Service**: JWT validation and user context
- **Lead Service**: Lead conversion integration
- **Pipeline Service**: Contact pipeline management
...
| Activity Retention Days | 90  | 365        |
...
1. **Database Connection Failures**
    ...
    ```bash
    kubectl logs deployment/iqscaffold-contact-service -n iqkvdev-dev-env
    ```
...
2. **Redis Connection Issues**
    ...
    kubectl exec -it deployment/iqscaffold-contact-service -n iqkvdev-dev-env -- \
...
3. **Check Configuration**
    ...
    ```bash
    kubectl describe configmap iqscaffold-contact-service-config -n iqkvdev-dev-env
    ```

4. **Test Health Endpoints**

    ```bash
    kubectl port-forward deployment/iqscaffold-contact-service 8081:8081 -n iqkvdev-dev-env
...
helm rollback iqscaffold-contact-service -n iqscaffold-production-env

# Or uninstall completely
helm uninstall iqscaffold-contact-service -n iqscaffold-production-env
```
